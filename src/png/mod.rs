...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        let mut current: usize = Png::STANDARD_HEADER.len();
        let header = value[..current].iter().as_slice();
        if header != Png::STANDARD_HEADER {
            return Err(PngParseError);
        }
        let length_size: usize = Chunk::LENGTH_SIZE as usize;
        let header_size: usize = length_size + Chunk::CHUNK_TYPE_SIZE as usize;
        let crc_size: usize = Chunk::CRC_SIZE as usize;
        ...
        loop {
            ...
            let length: usize = u32::from_be_bytes(
                value[current..current + length_size]
                    .try_into()
